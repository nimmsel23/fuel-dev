...
def _get_fs():
    ...
    logger.info(f"fuel-firestore: Firestore verbunden")
    return _fs

# ── Lokale Pfade ──────────────────────────────────────────────────────────────

def _get_uid(request_or_arg: Any = None) -> str:
    # Versucht, UID aus Request-Header oder Argument zu bekommen
    # Fallback auf Hardcoded UID "default", wenn keine UID übergeben wird
    if hasattr(request_or_arg, "headers"):
        return request_or_arg.headers.get("X-Fuel-UID", os.getenv("FUEL_FIRESTORE_UID", "default"))
    if isinstance(request_or_arg, str) and request_or_arg:
        return request_or_arg
    return os.getenv("FUEL_FIRESTORE_UID", "default")


def _nutrition_path(d: str, uid: str) -> Path:
    return FUEL_DATA_DIR / uid / "nutrition" / f"{d}.json"


def _journal_path(d: str, uid: str) -> Path:
    return FUEL_DATA_DIR / uid / "nutrition_journal" / f"{d}.md"


def _supplements_path(d: str, uid: str) -> Path:
    return FUEL_DATA_DIR / uid / "supplements" / "logs" / f"{d}.json"
...
def _sync_nutrition(d: str, direction: str, uid: str) -> dict:
    fs = _get_fs()
    local_path = _nutrition_path(d, uid)
...
def _sync_supplements(d: str, direction: str, uid: str) -> dict:
    fs = _get_fs()
    local_path = _supplements_path(d, uid)
...
def _sync_journal(d: str, direction: str, uid: str) -> dict:
    fs = _get_fs()
    local_path = _journal_path(d, uid)
...
def do_sync(d: str, direction: str, uid: str) -> dict:
    ...
    for name, fn in [
        ...
        try:
            results[name] = fn(d, direction, uid)
        except Exception as e:
