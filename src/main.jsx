          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="rounded-2xl border border-white/10 bg-slate-950/70 p-4 text-right">
                <div className="text-xs uppercase tracking-[0.2em] text-slate-400">Datum</div>
                <input
                  value={activeDate}
                  onChange={(event) => setActiveDate(event.target.value)}
                  type="date"
                  className="mt-2 rounded-xl border border-white/10 bg-slate-900 px-3 py-2 text-sm text-slate-100"
                />
              </div>
              <div className="rounded-2xl border border-white/10 bg-slate-950/70 p-4 text-right">
    ...
    <div className="grid gap-6 xl:grid-cols-[1.4fr_0.9fr]">
      <div className="grid gap-6">
        <div className="grid gap-4 md:grid-cols-3">
          ...
          <Card icon={Waves} title="Water" value={`${nutrition?.water_ml || 0} ml`} hint="Tageshydration" />
        </div>
...
function SettingsView() {
  return (
    <section className="grid gap-4 md:grid-cols-2">
      {[
        ["Fallback", "public/index.html bleibt als v1-Fallback aktiv."],
        ["Build", "Vite output landet nach /opt/fuel oder FUEL_BUILD_DIR."],
        ["Persistence", "Meals, Hydration und Journal schreiben jetzt direkt in data/nutrition und data/nutrition_journal."],
        ["Next", "Supplement-Logging, bessere Kalenderaggregation und Offline-Queue für POSTs."],
      ].map(([title, text]) => (
...
function FoodSearch({ onSelect }) {
  ...
  function search(q) {
    ...
    debounce.current = setTimeout(async () => {
      setLoading(true);
      try {
        const res = await fetchJson(`/nutrition/search?q=${encodeURIComponent(q)}&limit=15`);
