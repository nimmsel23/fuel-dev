...
export default function MicrosEstimator({ missingMeals, onClose }) {
  ...
  async function estimateMeals() {
    ...
    try {
      ...
      // Batch in chunks if many meals
      for (let i = 0; i < missingMeals.length; i++) {
        const mealName = missingMeals[i];
...
Beispiel: {"kcal": 450, "vitamin_c_mg": 12.5, "iron_mg": 2.1, ...}`;
        ...
        setProgress(Math.round(((i + 1) / missingMeals.length) * 100));
      }

      await mutation.mutateAsync(estimatedItems);
    } catch (e) {
      console.error(e);
      setError(e.message || "Fehler bei der Schätzung");
    } finally {
      setLoading(false);
    }
  }
        ...
        <div className="flex-1 overflow-y-auto p-5 text-sm text-slate-300">
          {success ? (
            ...
            <>
              ...
              {error && <div className="mb-4 rounded-lg bg-rose-500/10 p-3 text-rose-400">{error}</div>}

              <button
                onClick={estimateMeals}
