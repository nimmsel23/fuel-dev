...
export default function SupplementsView({ date, sup, catalog, suppLog }) {
  ...
  const intakeCountBySupplement = intakes.reduce((map, intake) => {
    const key = intake.supplement_id || intake.name;
    map[key] = (map[key] || 0) + 1;
    return map;
  }, {});
  ...
  const createSupplementMutation = useMutation({
    mutationFn: (values) => {
      const selected = catalog.find((item) => item.id === values.supplement_id) || { name: values.supplement_id };
  ...
  const deleteSupplementMutation = useMutation({
    mutationFn: ({ delete_id }) =>
      deleteJson(`/supplements/log/${delete_id}`),
    onSuccess: (_, values) => {
      queryClient.invalidateQueries({ queryKey: ["supp-log", values.date] });
      queryClient.invalidateQueries({ queryKey: ["supp-stats", values.date] });
  ...
  function quickUnlogSupplement(itemId) {
    ...
    const lastIntake = intakes.slice().reverse().find(i => (i.supplement_id || i.name) === itemId || (i.supplement_id || i.name) === catalog.find(c => c.id === itemId)?.name);
    if (lastIntake) {
