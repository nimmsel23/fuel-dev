...
import { useMicrosWeekly } from "../hooks/useMicrosWeekly.js";
...
export default function MicrosView() {
  ...
  const { data: weeklyData, isLoading } = useMicrosWeekly(weeks);
  ...
  return (
    <div className="space-y-6 p-4">
      ...
      <div className="overflow-x-auto">
        {isLoading ? (
          <div className="text-slate-400 text-sm py-4">Lade Daten...</div>
        ) : (
          <div
            ...
            {NUTRIENTS.map(({ key, label, unit }) => (
              <React.Fragment key={key}>
                ...
                {weeks.map((w, wi) => {
                  ...
                  const d = weeklyData?.[weekKey]?.rda_comparison?.[key];
                  return (
                    <Cell
                      key={`${key}-${wi}`}
      ...
      <p className="text-xs text-slate-600">
        Mikronährstoffe werden aus dem Micros-Katalog und Supplements geschätzt. Mahlzeiten ohne Eintrag zählen als 0.
      </p>
    </div>
  );
}
